#变量命名规范：
**格式：** `$component-state-property-size` 
**例子：** `$nav-link-disabled-color` and `$modal-content-box-shadow-xs`
# 目录结构
...
**通过修改这些变量来控制大多数引导元素的默认样式。主要集中在间隔上。
如果需要更多的变体，可以向$spacers映射中添加更多的条目。**
...
```
<h2 id="grid-containers">Grid containers</h2>
...
```
<h2 id="container-padding">Container padding</h2>
...
```
<h2 id="buttons-forms">Buttons + Forms</h2>
...
<h2 id="z-index-master-list"> Z-index master list</h2>
